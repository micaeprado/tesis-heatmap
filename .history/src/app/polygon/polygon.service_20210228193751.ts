...
export class PolygonService {
  ...
  getZonesPerPAge(page: number): Observable<Zone[]> {
    return this.http.get(this.urlEndPoint+ '/page/' +page).pipe(
      map( (response: any) => {
        //response as Cliente[]; -->vuelve response a una lista de clientes
        (response.content as Zone[]).map(zone =>{ //recorre la lista de clientes
